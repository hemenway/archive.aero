...
const CONFIG = {
  ...
  bundleUrl: 'https://data.archive.aero/sectionals/metadata-83ed9b48.bundle',
  ...
  // First view without ?lat/lng: the lower 48, fitted to the viewport.
  // The map's minZoom (6, MapController) clamps that fit and the era
  // archives start at z8, so today this opens on the central US at z6;
  // when the zoom floor drops the same bounds go coast to coast. Chart-
  // following views (share links, "View alone") pass center/zoom instead.
  initialView: { bounds: [[24.4, -125.0], [49.4, -66.9]] },
...
class CompositePMTilesLayer extends L.GridLayer {
  ...
  // True while any tile Leaflet still holds has not settled: the inverse of
  // GridLayer's private _noTilesToLoad, read directly because the 'load'
  // event is skipped when the last pending tile is pruned before it lands.
  hasPendingTiles() {
    for (const key in this._tiles) {
      if (!this._tiles[key].loaded) return true;
    }
    return false;
  }
...
class MapController {
  constructor(mapId, initialView = CONFIG.initialView) {
    ...
    // A bounds view (the lower-48 default) is fitted to the viewport and
    // clamped by minZoom; a center/zoom view (share links, chart focus) is
    // taken as given.
    if (initialView.bounds) {
      this.map.fitBounds(initialView.bounds, { animate: false });
    } else {
    ...
    // Loading-indicator bookkeeping (see _syncLoader): overlapping showFrame
    // calls are reference-counted here, and the composite layer's own tile
    // loads (pans and zooms into unpainted tiles) are read off the layer.
    this.pendingLoads = 0;
    ...
    // 'tileloadstart' fires per tile once it is registered as pending (the
    // batch-level 'loading' fires before any tile exists, which reads as
    // idle); 'load' fires once every tile held has settled. A tile pruned
    // mid-flight can leave 'load' unfired, so moveend/zoomend re-read the
    // layer's real state too — registered after addTo so they run after
    // the layer's own move handlers have queued their tiles.
    this.compositeLayer.on('tileloadstart load remove', () => this._syncLoader());
    this.map.on('moveend zoomend', () => this._syncLoader());
  ...
  async showFrame(selectedDateStr) {
    ...
    // indicator: it shows only if something is still loading 100ms in, and
    // hides only when the last concurrent load finishes (not when the first
    // returns).
    this.pendingLoads += 1;
    this._syncLoader();
    try {
  ...
  // True while a frame swap or a batch of tile loads is in flight.
  _chartsLoading() {
    return this.pendingLoads > 0 || this.compositeLayer.hasPendingTiles();
  }
...
export async function initApp() {
  ...
  const hideSplash = () => {
    ...
    const finish = () => { loadingSplash.hidden = true; };
  ...
  // Resolve the initial view BEFORE creating the map so the first basemap
  // tiles fetched are the ones kept. A share link's lat/lng are honored even
  // when zoom is missing or invalid; otherwise the map opens on the lower 48
  // (CONFIG.initialView). Nothing geolocates the visitor on load (dropped
  // 2026-09-25); only the locate button asks where they are.
