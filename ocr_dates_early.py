#!/usr/bin/env python3
"""
OCR effective dates for early sectional chart records.
Uses the same file search logic as slicer.py with early_master_dole.csv.

Crops the bottom region of each chart image and uses Tesseract OCR to extract
date text for legacy chart formats.

Usage:
    python3 ocr_dates_early.py                      # Process all records
    python3 ocr_dates_early.py --location Boston    # Filter by location
    python3 ocr_dates_early.py --before-year 1970   # Only process dates before 1970
    python3 ocr_dates_early.py --missing-only       # Only process records with blank Date
    python3 ocr_dates_early.py --output results.csv # Save results to CSV
    python3 ocr_dates_early.py --update             # Update early_master_dole.csv Date column
"""
...
CSV_FILE = SCRIPT_DIR / 'early_master_dole.csv'
SOURCE_DIR = Path('/Volumes/projects/rawtiffs')
...
# Date patterns for early charts.
DATE_PATTERN = re.compile(
    rf'\b(\d{{1,2}})\s*({MONTH_TOKEN})\.?\s+(\d{{4}})\b',
    re.IGNORECASE
)
...
BOTTOM_FRACTION_START = 0.80

FIELD_ALIASES = {
    'tif_filename': ('tif_filename', 'filename', 'Filename'),
    'download_link': ('download_link', 'Download link'),
    'location': ('Location', 'location', 'geo_where'),
    'date': ('Date', 'date', 'start_date', 'Start Date'),
    'end_date': ('end_date', 'End Date'),
    'edition': ('edition', 'Edition', 'Web Edition Number'),
}

_SOURCE_INDEX_CACHE = {}
_SOURCE_INDEX_LOCK = Lock()
...
def _get_field(record, alias_key, default=''):
    """Read a record field from the first known alias."""
...
def _set_field(record, alias_key, value):
    """Write a record field to an existing alias or the first alias if missing."""
    for key in FIELD_ALIASES[alias_key]:
        if key in record:
            record[key] = value
            return key
    first = FIELD_ALIASES[alias_key][0]
    record[first] = value
    return first


def _record_tif_filename(record):
    return _get_field(record, 'tif_filename', '')


def _record_download_link(record):
    return _get_field(record, 'download_link', '')


def _record_location(record):
    return _get_field(record, 'location', '')


def _record_date(record):
    return _get_field(record, 'date', '')


def _record_end_date(record):
    return _get_field(record, 'end_date', '')


def _record_edition(record):
    return _get_field(record, 'edition', '')
...
def _path_rank(path, root):
    """Rank shorter/shallower paths first for deterministic fallback selection."""
    try:
...
def _source_index(source_dir):
    """Build and cache recursive file/dir name indexes for fallback lookups."""
    ...
    with _SOURCE_INDEX_LOCK:
        ...
        print(f"  Building recursive source index under {root}...")
        files_by_name = {}
        dirs_by_name = {}
        ...
        for dirpath, dirnames, filenames in os.walk(root, onerror=_on_walk_error):
            ...
            for dirname in dirnames:
                ...
                dirs_by_name.setdefault(dirname.lower(), []).append(path)
            for filename in filenames:
                path = current_dir / filename
                files_by_name.setdefault(filename.lower(), []).append(path)

        for bucket in files_by_name.values():
            bucket.sort(key=lambda p: _path_rank(p, root))
        for bucket in dirs_by_name.values():
            bucket.sort(key=lambda p: _path_rank(p, root))

        index = {
            'root': root,
            'files_by_name': files_by_name,
            'dirs_by_name': dirs_by_name,
        }
        _SOURCE_INDEX_CACHE[key] = index
        print(
            f"  Source index ready: {sum(len(v) for v in files_by_name.values())} files, "
            f"{sum(len(v) for v in dirs_by_name.values())} directories"
        )
        return index


def _lookup_files_recursive(source_dir, filename):
    """Return files matching basename anywhere under source_dir."""
    if not filename:
        return []
    index = _source_index(source_dir)
    return index['files_by_name'].get(filename.lower(), [])


def _lookup_dirs_recursive(source_dir, dirname):
    """Return directories matching basename anywhere under source_dir."""
    if not dirname:
        return []
    index = _source_index(source_dir)
    return index['dirs_by_name'].get(dirname.lower(), [])
...
def resolve_source_file(record, source_dir=None):
    ...
    tif_filename = _record_tif_filename(record)
    download_link = _record_download_link(record)
    ...
    # Case 8: Recursive directory lookup from source root and all subfolders.
    stem = Path(tif_filename).stem
    dir_names = _dedupe_names([
        tif_filename,
        Path(tif_filename).name,
        stem,
    ])
    for dir_name in dir_names:
        for found_dir in _lookup_dirs_recursive(source_dir, dir_name):
            tifs = _collect_tif_candidates(found_dir)
            if tifs:
                return tifs[0], '.tif'

    # Case 9: Recursive file lookup by basename (root + any subfolder).
    base_name = Path(tif_filename).name
    ...
    if stem:
        ...
        if url_ext:
            file_names.append(f'{stem}{url_ext}')

    for file_name in _dedupe_names(file_names):
        matches = list(_lookup_files_recursive(source_dir, file_name))
        if not matches:
            continue

        tif_matches = [m for m in matches if m.suffix.lower() in ('.tif', '.tiff')]
        if tif_matches:
            preferred = _north_sibling_if_available(tif_matches[0])
            return preferred, preferred.suffix.lower()

        # If only ZIP/PDF/etc. matched, return the first ranked candidate.
        best = matches[0]
...
def _candidate_src_windows(width, height):
    """Return candidate (x, y, w, h) windows focused on the bottom chart band."""
    if height <= 0 or width <= 0:
        return []

    # Primary search in the bottom 20% of the image.
    y_bands = [
        (BOTTOM_FRACTION_START, 1.00),
        (0.76, 0.98),
        (0.72, 0.96),
    ]
    x_bands = [
    ...
    for y0f, y1f in y_bands:
        for x0f, x1f in x_bands:
            fractions.append((x0f, y0f, x1f, y1f))

    # Nearby fallback windows for charts where legends are slightly above the bottom edge.
    fractions.extend([
        (0.00, 0.66, 0.45, 0.90),
        (0.26, 0.66, 0.74, 0.90),
        (0.55, 0.66, 1.00, 0.90),
...
def parse_dates_from_text(text):
    ...
    # Early charts often use MONTH YYYY (no day) labels.
    if not effective_date and month_years:
        m, y = month_years[0]
        effective_date = format_month_year(m, y)
    if (not to_date) and len(month_years) >= 2 and re.search(r'\bTO\b', normalized):
        m, y = month_years[1]
        to_date = format_month_year(m, y)

    # Last fallback for sparse labels (e.g., "1953").
    if not effective_date and years:
        effective_date = years[0]
...
def format_month_year(month_str, year):
    """Convert month/year to YYYY-Month format (e.g., 1969-March)."""
    month = _month_to_number(month_str)
    if not month:
        return None
    try:
        ...
        return None
    return f"{y:04d}-{MONTH_NAMES[month]}"


def _month_to_number(month_str):
    if not month_str:
        return None
    cleaned = re.sub(r'[^A-Z]', '', str(month_str).upper())
    if not cleaned:
        return None
...
def _parse_fuzzy_date(value):
    """
    Parse flexible date strings used by early_master_dole:
    YYYY-MM-DD, YYYY-Month, or YYYY.
    Returns (year, month, day, precision) or None.
    """
    if not value:
        return None
    s = str(value).strip()
    ...
    if m:
        return int(m.group(1)), int(m.group(2)), int(m.group(3)), 3

    m = re.match(r'^(\d{4})-([A-Za-z]+)$', s)
    if m:
        month = _month_to_number(m.group(2))
        if month:
            return int(m.group(1)), month, None, 2

    m = re.match(r'^(\d{4})$', s)
    if m:
        return int(m.group(1)), None, None, 1

    # Handle rare OCR output like "March 1969".
    m = re.match(r'^([A-Za-z]+)\s+(\d{4})$', s)
    if m:
        month = _month_to_number(m.group(1))
        if month:
            return int(m.group(2)), month, None, 2

    return None


def _date_match_status(csv_value, ocr_value):
    """
    Compare CSV and OCR dates with partial precision support.
    Returns one of: full_match, start_match, mismatch, None.
    """
    if not csv_value or not ocr_value:
        return None
    if csv_value == ocr_value:
        return 'full_match'

    csv_parts = _parse_fuzzy_date(csv_value)
    ocr_parts = _parse_fuzzy_date(ocr_value)
    if not csv_parts or not ocr_parts:
        return 'mismatch'

    csv_y, csv_m, csv_d, csv_precision = csv_parts
    ocr_y, ocr_m, ocr_d, ocr_precision = ocr_parts
    if csv_y != ocr_y:
        return 'mismatch'
    if csv_precision >= 2 and ocr_precision >= 2 and csv_m != ocr_m:
        return 'mismatch'
    if csv_precision >= 3 and ocr_precision >= 3 and csv_d != ocr_d:
        return 'mismatch'

    if csv_precision == ocr_precision:
        return 'full_match'
    return 'start_match'


def _parse_iso_date(value):
    """Parse flexible date strings into a datetime anchor for scoring."""
    parts = _parse_fuzzy_date(value)
    if not parts:
        return None
...
def record_year(record):
    ...
    """
    for value in (_record_date(record), _record_end_date(record)):
...
def process_record(record, index, total, source_dir=None):
    ...
    csv_date = _record_date(record)
    csv_end_date = _record_end_date(record)
    csv_edition = _record_edition(record)
    ...
    for img in images:
        ...
        for text in texts:
            ...
            if effective and to_date and score >= HIGH_CONFIDENCE_PARSE_SCORE:
                break
            if effective and csv_date and _date_match_status(csv_date, effective) in ('full_match', 'start_match'):
                # If start date matches expected CSV date, stop searching early.
                to_ok = (not csv_end_date) or (to_date and _date_match_status(csv_end_date, to_date) in ('full_match', 'start_match'))
                if to_ok:
                    break
        if timed_out:
            break
        if best_candidate and best_candidate['effective'] and best_candidate['to_date'] \
            ...
            break
        if best_candidate and csv_date and _date_match_status(csv_date, best_candidate['effective']) in ('full_match', 'start_match'):
            to_ok = (not csv_end_date) or (
                best_candidate['to_date']
                and _date_match_status(csv_end_date, best_candidate['to_date']) in ('full_match', 'start_match')
            )
            if to_ok:
                break
    ...
    if effective and csv_date:
        start_match = _date_match_status(csv_date, effective)
        if start_match in ('full_match', 'start_match'):
            if to_date and csv_end_date:
                end_match = _date_match_status(csv_end_date, to_date)
                if end_match in ('full_match', 'start_match'):
                    result['match'] = 'full_match' if start_match == 'full_match' and end_match == 'full_match' else 'start_match'
                else:
...
def print_record_result(pos, total, record, result, verbose=False):
    ...
    tif_filename = _record_tif_filename(record)
    csv_date = _record_date(record)
    prefix = f"  [{pos+1}/{total}] {location} {tif_filename} ({csv_date})..."
    ...
    if result['status'] == 'ok':
        ...
        print(f"{prefix} OCR: {ocr_eff} to {ocr_to}{match_icon}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="OCR early chart dates from bottom chart regions",
    ...
    parser.add_argument(
        ...
        help="Only process records with blank Date values"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Update CSV date fields with OCR results (for missing values)"
    )
    parser.add_argument(
    ...
    for i, record in enumerate(records):
        tif_name = _record_tif_filename(record)
        if not tif_name:
            continue
        if args.location:
    ...
    if mismatches:
        ...
        for r in mismatches:
            print(f"  {r['location']} {r['tif_filename']}:")
    ...
    if args.update:
        ...
        for r in results:
            ...
            rec = records[idx]
            current_date = _record_date(rec)
            if r['ocr_effective'] and not current_date:
                _set_field(rec, 'date', r['ocr_effective'])
                updated += 1
            current_end = _record_end_date(rec)
            if r['ocr_to'] and current_end == '':
                if any(alias in rec for alias in FIELD_ALIASES['end_date']):
                    _set_field(rec, 'end_date', r['ocr_to'])
                    updated += 1
            current_edition = _record_edition(rec)
            if r['ocr_edition'] and current_edition == 'Unknown':
                _set_field(rec, 'edition', r['ocr_edition'])
                updated += 1
